...
export default function CardBuilderPanel() {
  ...
  // Fetch token balance when user has wager access (not just when picker opens)
  useEffect(() => {
    if (!heatModeAccess || tokenBalance !== null) return;
                  ...
                  <>
                    ...
                    <div className="flex gap-1">
                      {[25, 50, 100].map((amt) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            disabled={isDisabled}
                            onClick={() => setWager(wager === amt ? null : amt)}
                            className={cn(
                      ...
                      <button
                        ...
                        onClick={() => setWager(wager === tokenBalance ? null : (tokenBalance ?? 0))}
                        className={cn(
                          "rounded-md px-2 py-1 text-[11px] font-bold transition-colors",
                          wager === tokenBalance
                            ? "bg-orange-500/20 text-orange-400"
